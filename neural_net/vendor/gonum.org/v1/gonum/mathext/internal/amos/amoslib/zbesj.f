...
C         FUNCTIONS
C
C         CY(I)=EXP(-ABS(Ytr))*J(FNU+I-1,Z)   I = 1,...,N , Ytr=AIMAG(Z)
C
...
C                        = 2  RETURNS
C                             CY(I)=J(FNU+I-1,Z)EXP(-ABS(Ytr)), I=1,...,N
...
C                    CY(I)=J(FNU+I-1,Z)EXP(-ABS(Ytr))  I=1,...,N
C                    DEPENDING ON KODE, Ytr=AIMAG(Z).
...
C
C              J(-FNU,Z) = J(FNU,Z)*COS(PI*FNU) - Ytr(FNU,Z)*SIN(PI*FNU)
C
...
C         Ytr(FNU,Z) DOMINATES IN MAGNITUDE WITH A LARGE POSITIVE POWER OF
